...
{
	i2c_oled_master_gpio_init();		// 初始化SCL(IO14)、SDA(IO2)
}
//-----------------------------------------------------------
...
char OLED_Write_Command(unsigned char OLED_Byte)
{
	...
	i2c_oled_master_writeByte(0x78);			// 选择总线上的OLED[0111 100X B]
	if(i2c_oled_master_checkAck()==0)
	{i2c_oled_master_stop(); return 0;}

	i2c_oled_master_writeByte(0x00);			// [0x00]表示下一字节写入的是[指令]
	if(i2c_oled_master_checkAck()==0)
	{i2c_oled_master_stop(); return 0;}

	i2c_oled_master_writeByte(OLED_Byte);	// [指令代码]
	if(i2c_oled_master_checkAck()==0)
	{i2c_oled_master_stop(); return 0;}

	i2c_oled_master_stop();					// 发送停止信号
...
char OLED_Write_Data(unsigned char OLED_Byte)
{
	...
	i2c_oled_master_writeByte(0x40);			// [0x40]表示下一字节写入的是[数据]
	if(i2c_oled_master_checkAck()==0)
	{i2c_oled_master_stop(); return 0;}

	i2c_oled_master_writeByte(OLED_Byte);	// [具体数据]
	if(i2c_oled_master_checkAck()==0)
	{i2c_oled_master_stop(); return 0;}
...
{
	unsigned char N_IP_Byte = 0;		// IP字节序号
...
uint8_t i2c0_pcf8591_adc_read(uint8_t ch)
{
        ...
        i2c_oled_master_writeByte(0x91);                 // 选择总线上的OLED[0111 100X B]
        if(i2c_oled_master_checkAck()==0)
        {i2c_oled_master_stop(); return 0;}
        val = i2c_oled_master_readByte();        // [指令代码]
